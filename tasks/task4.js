...
function getMostFuelEfficientCars(cars, brand) {
   let carsfuelConsumptionMoreSix = cars.filter(
      (car) => car.fuelConsumption >= 6
   );
   if (carsfuelConsumptionMoreSix.length > 0) {
      return [];
   }
   let carsByBrand = cars.filter((car) => car.brand === brand);
   let carsLessThanSixLitters = carsByBrand.filter(
      (car) => car.fuelConsumption < 6
   );
   let mostEconomical = carsLessThanSixLitters.sort(
