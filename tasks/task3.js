...
function addSuccessPercent(olympicRepresentation) {
   return olympicRepresentation.map((olimpicItem) => {
      const percent = (olimpicItem.medals / olimpicItem.athletes) * 100;
      const formattedPercent =
         olimpicItem.athletes === 0 ? "0%" : percent.toFixed(1) + "%";
      return {
         ...olimpicItem,
         percentOfSuccess: formattedPercent,
      };
   });
}
